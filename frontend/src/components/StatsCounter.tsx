...
const StatItem = ({ label, value, suffix, inView, icon: Icon, index, textColor, iconColor }: { label: string; value: number; suffix: string; inView: boolean; icon: React.ElementType; index: number; textColor: string; iconColor: string }) => {
  ...
  return (
    <div className="text-center min-w-0 flex flex-col items-center px-1 py-2.5 sm:px-2 sm:py-3 md:px-4 md:py-5 opacity-0 animate-fade-in-up" style={{ animationDelay: `${index * 150}ms`, animationFillMode: 'forwards' }}>
      <div className="w-6 h-6 sm:w-7 sm:h-7 md:w-9 md:h-9 rounded-lg md:rounded-xl bg-primary-foreground/10 border border-primary-foreground/20 flex items-center justify-center mb-1 md:mb-2">
        <Icon className="w-3 h-3 sm:w-3.5 sm:h-3.5 md:w-4 md:h-4" style={{ color: iconColor }} />
      </div>
      <div className="text-[12px] min-[380px]:text-[13px] sm:text-2xl md:text-[1.75rem] font-extrabold tracking-tight leading-none tabular-nums" style={{ color: textColor }}>
        {count.toLocaleString()}{suffix}
      </div>
      <div className="text-[8px] min-[380px]:text-[9px] sm:text-xs font-semibold mt-1 leading-tight whitespace-nowrap max-w-full truncate" style={{ color: textColor, opacity: 0.75 }}>{label}</div>
    </div>
  );
};
...
const StatsCounter = () => {
  ...
  return (
    <section ref={ref} className="py-3 md:py-6 bg-background">
      <div className="w-full">
        ...
        >
          ...
          <div className="relative z-10 grid grid-cols-4 divide-x divide-primary-foreground/10">
