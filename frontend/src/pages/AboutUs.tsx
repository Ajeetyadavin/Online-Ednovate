    ...
    <div className="bg-[#f7f7f5] text-slate-900 [font-family:-apple-system,BlinkMacSystemFont,'SF_Pro_Text','SF_Pro_Display','Helvetica_Neue',Arial,sans-serif] [&_h1]:[font-family:-apple-system,BlinkMacSystemFont,'SF_Pro_Display','SF_Pro_Text','Helvetica_Neue',Arial,sans-serif] [&_h2]:[font-family:-apple-system,BlinkMacSystemFont,'SF_Pro_Display','SF_Pro_Text','Helvetica_Neue',Arial,sans-serif]">
      <section className="relative overflow-hidden bg-white">
        ...
        <div className="mx-auto grid max-w-6xl gap-8 px-4 py-12 md:gap-10 md:py-14 lg:grid-cols-[1.08fr_0.92fr] lg:px-6 lg:py-16">
          <div className="relative z-10 flex flex-col justify-center text-left">
            <h1 className="mt-5 max-w-3xl text-3xl font-bold leading-tight tracking-[0.01em] text-slate-950 sm:text-4xl lg:text-5xl">
              Learn with clarity, confidence, and the right kind of fun.
            </h1>
            <p className="mt-4 max-w-2xl text-[15px] leading-8 text-slate-700 sm:text-base">
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <Button asChild className="h-11 rounded-xl px-5 text-sm font-semibold text-white shadow-lg" style={{ backgroundColor: BRAND_ORANGE, boxShadow: "0 10px 26px -12px rgba(231,71,35,0.65)" }}>
                <Link to="/packages">
                  Explore Courses <ArrowRight className="ml-2 h-4 w-4" />
                </Link>
              </Button>
              <Button asChild variant="outline" className="h-11 rounded-xl px-5 text-sm font-semibold" style={{ borderColor: BRAND_BLUE, color: BRAND_BLUE }}>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <article className="overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
            <div className="flex items-center gap-3">
              <div className="rounded-2xl p-3" style={{ backgroundColor: "rgba(38,71,150,0.12)", color: BRAND_BLUE }}><Target className="h-5 w-5" /></div>
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.2em]" style={{ color: BRAND_BLUE }}>Our Vision</p>
                <h2 className="text-2xl font-bold tracking-[0.01em] text-slate-950">Build a learning platform students trust.</h2>
              </div>
            </div>
                ...
                <ul className="mt-4 space-y-2 text-sm text-slate-700">
                  {visionPoints.map((point) => (
                    <li key={point} className="flex gap-2">
                      <BadgeCheck className="mt-0.5 h-4 w-4 shrink-0" style={{ color: BRAND_ORANGE }} />
                      <span>{point}</span>
                    </li>
                  ))}
                </ul>
          ...
          <article className="overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
            <div className="flex items-center gap-3">
              <div className="rounded-2xl p-3" style={{ backgroundColor: "rgba(231,71,35,0.12)", color: BRAND_ORANGE }}><BookOpen className="h-5 w-5" /></div>
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.2em]" style={{ color: BRAND_ORANGE }}>Our Mission</p>
                <h2 className="text-2xl font-bold tracking-[0.01em] text-slate-950">Set the benchmark for commerce coaching.</h2>
              </div>
            </div>
            <div className="mt-5 grid gap-5 md:grid-cols-[0.95fr_1.05fr] md:items-center">
              ...
              <div>
                ...
                <ul className="mt-4 space-y-2 text-sm text-slate-700">
                  {missionPoints.map((point) => (
                    <li key={point} className="flex gap-2">
                      <BadgeCheck className="mt-0.5 h-4 w-4 shrink-0" style={{ color: BRAND_BLUE }} />
                      <span>{point}</span>
                    </li>
                  ))}
                </ul>
      ...
      <section className="border-y border-slate-200 bg-white py-12">
        <div className="mx-auto grid max-w-6xl gap-8 px-4 lg:grid-cols-[0.9fr_1.1fr] lg:px-6">
          <div className="flex flex-col justify-center">
            <p className="text-xs font-bold uppercase tracking-[0.22em]" style={{ color: BRAND_BLUE }}>Meet Our CEO</p>
            <h2 className="mt-3 text-3xl font-bold tracking-[0.01em] text-slate-950 sm:text-4xl">CA Ashish Shah</h2>
            ...
            <ul className="mt-6 space-y-3 text-sm text-slate-700">
              {ceoHighlights.map((point) => (
                <li key={point} className="flex gap-2">
                  <Users className="mt-0.5 h-4 w-4 shrink-0" style={{ color: BRAND_BLUE }} />
                  <span>{point}</span>
                </li>
              ))}
            </ul>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <Button asChild className="h-11 rounded-xl px-5 text-sm font-semibold text-white" style={{ backgroundColor: BRAND_BLUE }}>
                <Link to="/contact-us">Talk to the Team</Link>
              </Button>
              <Button asChild variant="outline" className="h-11 rounded-xl px-5 text-sm font-semibold hover:bg-slate-50" style={{ borderColor: BRAND_ORANGE, color: BRAND_ORANGE }}>
      ...
      <section className="mx-auto max-w-6xl px-4 py-12 lg:px-6">
        <div className="grid gap-8 lg:grid-cols-[1fr_0.95fr] lg:items-start">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.22em]" style={{ color: BRAND_BLUE }}>Why Ednovate</p>
            <h2 className="mt-3 text-3xl font-bold tracking-[0.01em] text-slate-950 sm:text-4xl">Built for commerce students who want results.</h2>
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              {whyPoints.map((point) => (
                <div key={point} className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
                  <HeartHandshake className="h-4 w-4 shrink-0" style={{ color: BRAND_ORANGE }} />
          ...
          <aside className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <p className="text-xs font-bold uppercase tracking-[0.22em]" style={{ color: BRAND_ORANGE }}>What students get</p>
            ...
            <div className="mt-6 rounded-xl border border-slate-200 bg-slate-50 p-5 text-slate-800">
              ...
              <Button asChild className="mt-4 h-10 rounded-xl px-4 text-sm font-semibold text-white" style={{ backgroundColor: BRAND_ORANGE }}>
                <Link to="/packages">
      ...
      <section className="mx-auto max-w-6xl px-4 pb-14 lg:px-6">
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
          <div className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.22em]" style={{ color: BRAND_BLUE }}>Our promise</p>
              <h2 className="mt-3 text-3xl font-bold tracking-[0.01em] text-slate-950 sm:text-4xl">Learning that feels structured, human, and effective.</h2>
              <p className="mt-4 max-w-2xl text-sm leading-8 text-slate-600 sm:text-base">
