...
type AdminCategoriesProps = {
  embedded?: boolean;
  onManageSubjects?: (id: string, name: string, type: 'course' | 'level') => void;
};
...
export default function AdminCategories({ embedded = false, onManageSubjects }: AdminCategoriesProps) {
  ...
  return (
    <div className="space-y-5 font-['Inter']">
      {/* ─── Header ─────────────────────────────────────── */}
      <div className={`flex flex-wrap items-center gap-3 ${embedded ? "pt-1" : ""}`}>
        <div className="flex items-center gap-2.5">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary/10">
            <Tag className="h-5 w-5 text-primary" />
          </div>
          <div>
            <h1 className="text-xl font-bold text-slate-900">{embedded ? "Course & Level Master" : "Categories"}</h1>
            <p className="text-xs text-slate-400">{embedded ? "Organise taxonomy as Course and Level" : "Organise courses into categories and subcategories"}</p>
          </div>
        </div>
        ...
        <div className="flex gap-2 ml-2">
          ...
          <span className="rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold text-primary">{embedded ? "Courses" : "Root"}: {rootCount}</span>
          <span className="rounded-full bg-blue-100 px-3 py-1 text-xs font-semibold text-blue-700">{embedded ? "Levels" : "Subcategories"}: {subCount}</span>
        ...
        <div className="ml-auto flex items-center gap-2">
          <div className="relative">
            ...
            <Input className="h-9 w-56 rounded-xl border-slate-200 pl-9 text-xs" placeholder={embedded ? "Search course/level..." : "Search categories..."} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
          </div>
          <Button size="sm" className="h-9 gap-1.5 rounded-xl px-4 text-xs font-semibold" onClick={openAdd}>
            <Plus className="h-3.5 w-3.5" />{embedded ? "New Course" : "New Category"}
          </Button>
        </div>
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="grid grid-cols-[2fr_auto_auto_auto_auto] gap-4 border-b border-slate-100 bg-slate-50 px-6 py-3 text-[10px] font-bold uppercase tracking-wider text-slate-400">
          <span>{embedded ? "Course / Level" : "Category"}</span>
          <span>Slug</span>
          <span>{embedded ? "Course" : "Parent"}</span>
          <span>Status</span>
          <span>Actions</span>
        </div>
        ...
        {filteredCategories.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-sm font-semibold text-slate-400">{embedded ? "No course/level found" : "No categories found"}</p>
            <p className="mt-1 text-xs text-slate-300">{embedded ? "Create your first Course or Level" : "Create your first category to organise courses"}</p>
            <Button size="sm" className="mt-4 gap-1.5 rounded-xl text-xs" onClick={openAdd}><Plus className="h-3.5 w-3.5" />{embedded ? "New Course" : "New Category"}</Button>
          </div>
        ) : (
                  ...
                  <div className="grid grid-cols-[2fr_auto_auto_auto_auto] items-center gap-4 px-6 py-3.5 transition-colors hover:bg-slate-50/70">
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        {subs.length > 0 && (
                          <p className="text-[10px] text-slate-400">{subs.length} {embedded ? "level" : "subcategor"}{subs.length === 1 ? "y" : "ies"}</p>
                        )}
                      </div>
                      <span className="rounded-full bg-primary/10 px-2 py-0.5 text-[10px] font-bold text-primary">{embedded ? "Course" : "Root"}</span>
                    </div>
                    {/* Slug */}
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <button type="button" onClick={() => handleDelete(cat.id)} title="Delete"
                        ...
                        {embedded && onManageSubjects && (
                          <button type="button" onClick={() => onManageSubjects(cat.id, cat.name, 'course')}
                            className="flex h-7 w-7 items-center justify-center rounded-lg text-slate-400 transition-colors hover:bg-blue-50 hover:text-blue-600" title="Manage Subjects">
                  ...
                  {subs.map((sub) => (
                    <div key={sub.id} className="grid grid-cols-[2fr_auto_auto_auto_auto] items-center gap-4 border-t border-slate-50 bg-slate-50/40 px-6 py-3 transition-colors hover:bg-slate-50/80">
                      ...
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <span className="rounded-full bg-blue-50 px-2 py-0.5 text-[10px] font-bold text-blue-600">{embedded ? "Level" : "Sub"}</span>
                      </div>
                      {/* Slug */}
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <button type="button" onClick={() => handleDelete(sub.id)} title="Delete"
                          ...
                          {embedded && onManageSubjects && (
                            <button type="button" onClick={() => onManageSubjects(sub.id, sub.name, 'level')}
                              className="flex h-7 w-7 items-center justify-center rounded-lg text-slate-400 transition-colors hover:bg-blue-50 hover:text-blue-600" title="Manage Subjects">
                              <BookOpen className="h-3.5 w-3.5" />
                            </button>
                          )}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-md overflow-hidden rounded-2xl border-slate-100 p-0 shadow-2xl">
          <DialogHeader className="border-b border-slate-100 px-6 py-4">
            <DialogTitle className="text-base font-bold text-slate-900">{editingId ? (embedded ? "Edit Course / Level" : "Edit Category") : (embedded ? "New Course / Level" : "New Category")}</DialogTitle>
          </DialogHeader>

          <div className="space-y-4 px-6 py-5">
            {/* Name */}
            <div className="space-y-1.5">
              <FL>{embedded ? "Course / Level Name *" : "Category Name *"}</FL>
              <Input className={fCls} placeholder={embedded ? "e.g., CA Inter or Group 1" : "e.g., Taxation"} autoFocus value={form.name}
            ...
            {/* Parent */}
            <div className="space-y-1.5">
              <FL>{embedded ? "Parent Course" : "Parent Category"}</FL>
              <select className="h-9 w-full rounded-xl border border-slate-200 bg-white px-3 text-xs focus:outline-none focus:ring-2 focus:ring-primary/40"
                ...
                <option value="none">{embedded ? "None — create as Course" : "None — create as root category"}</option>
          ...
          <div className="flex justify-end gap-2 border-t border-slate-100 bg-slate-50/60 px-6 py-4">
            ...
            <Button size="sm" className="gap-1.5 rounded-xl px-5 text-xs font-semibold" onClick={() => void handleSave()} disabled={isSaving}>
              {isSaving ? "Saving..." : editingId ? (embedded ? "Update Course / Level" : "Update Category") : (embedded ? "Create Course / Level" : "Create Category")}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
